...
# inputs
db = 'B.sqlite' # database to analyze, assumed to be in results directory


# This is order that items will be plotted, only items included will be plotted
tech_rename <- c('EC_BATT'="'EC_BATT'",
                 'EC_BIO'="'EC_BIO'",
                 'EC_COAL'="'EC_COAL'",
                 'EC_DSL_CC'="'EC_DSL_CC'",
                 'EC_OIL_CC'="'EC_OIL_CC'",
...
# Use a predefined a palette https://www.datanovia.com/en/blog/top-r-color-palettes-to-know-for-great-data-visualization/
if (length(tech_rename)<=8) {
    tech_palette <- brewer.pal(n=length(tech_rename),name="Set1")     
  } else {
    tech_palette <- colorRampPalette(brewer.pal(8, "Set1"))(length(tech_rename))
}

fuel_rename <- c('IMPBIO'="'Biomass'",
                 'IMPCOAL'="'Coal'",
                 'IMPDSL'="'Diesel'",
                 'IMPOIL'="'Oil'",
...
if (length(fuel_rename)<=8) {
  fuel_palette <- brewer.pal(n=length(fuel_rename),name="Set1")     
} else {
  fuel_palette <- colorRampPalette(brewer.pal(8, "Set1"))(length(fuel_rename))
}
...
demand_palette <- brewer.pal(n=length(tbl1$periods),name="Blues") # predefined palette # http://www.cookbook-r.com/Graphs/Colors_(ggplot2)/
